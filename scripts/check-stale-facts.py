...
SKIP_PARTS = {"node_modules", ".git", "copy-baseline", "events", "backups",
              ...
              "preserved",
              # Local-only print artifacts (.gitignore:20), so CI never sees them and a
              # developer running this locally got a DIFFERENT answer from the runner --
              # two HIGH findings that could not be reproduced in CI and could not be
              # cleared by any commit. The content is dated memo prose ("a year ago today
              # -- 2025-07-30"), which is correct history, not a stale current claim.
              #
              # The general hazard, worth stating once: this guard walks the FILESYSTEM,
              # so anything gitignored under public/ is visible here and invisible to CI.
              # A guard that disagrees with CI about what exists teaches people to trust
              # neither. (Those files never deploy -- Actions builds from a clean
              # checkout, so rsync never sees them.)
              "_review"}
...
SKIP_FILES = {
    ...
    # A VERBATIM MIRROR of pdoom1's GitHub issues, regenerated by update-game-data.yml.
    # Its version literals are other people's words -- on 2026-08-01 a bot pulled in an
    # issue body quoting Pip's own "v0.13 playtest 2026-07-24" note, which scored HIGH as a
    # fallback literal and turned main and every open PR red. Nobody could fix it here: the
    # only edit that would clear it is editing an issue in ANOTHER repo, and the next sync
    # would reinstate it anyway.
    #
    # The rule this guard enforces is "the site must not present a stale fact as CURRENT".
    # An attributed, dated quotation is not that. /issues/ renders these bodies, so the text
    # does reach a reader -- but it reaches them as "pdoom1 issue #N said this", which is
    # true, and re-stamping the quote to 0.13.2 would forge what someone wrote. Same
    # reasoning as `preserved` and board-liveness.json above: an observation of what was
    # said or seen is not a claim about what is current.
    #
    # scripts/test-weekly-league-boundary.py already carries this exact exemption, for this
