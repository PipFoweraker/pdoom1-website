#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Build a Jira import payload for the coordination seat.
...
321: ("content-honesty", 1, None, [],
      "A probe POSTed to production returned ok:true and showed the sender 'Report Submitted!' while nothing "
      "arrived, because the domain publishes no SPF record and mail() sends from a host Google does not "
      "authorise -- the site is telling people it received them and losing them."),

317: ("content-honesty", 1, None, [],
      "Data contract validation is failing on a schedule, so a seam that publishes to the site is either "
      "off-schema or self-contradicting and nothing downstream knows."),

328: ("release-pipeline", 1, None, [],
      "The daily Plausible-to-git snapshot is failing and the analytics VPS has no other backup, so every day "
      "it stays broken is a day of traffic history that exists in exactly one place."),

316: ("board-and-league", 1, None, [],
      "The epoch-drift check cannot compare the key this site declares against the one the shipped client posts "
      "to, and the last time that condition went unread real scores landed for two days on a board nobody "
      "displayed."),

308: ("community-and-comms", 2, None, [],
      "Pip asked on 2026-08-11 for one art asset published with an explicit AI-generated declaration and a "
      "gallery entry showing its provenance; the clock he set was that same day and nothing has been published "
      "under those terms since."),

303: ("board-and-league", 2, None, [],
      "The blessing-consistency guard reads regularised_from as if it were the current epoch when it records a "
      "fixed historical boundary, so two of its seven findings are the guard's fault and acting on them would "
      "change correct data."),

324: ("community-and-comms", 3, None, [],
      "A /pdoom/ intro page is in copy review in the personal-homepage repo and this repo was pinged so the "
      "same introduction is not written twice; no work is being asked of us."),

309: ("site-design-and-copy", 3, None, [],
      "Pip named a measurement problem -- he cannot tell who reads everything and leaves from who would stay if "
      "there were more -- and thinks the fix is more densely interlinked pages; recorded as thinking aloud, not "
      "an instruction."),
