# The league epoch anomaly — pre-history before 2026-07-31

**Status:** deliberate. Nothing described here is a bug awaiting a fix.
**Written:** 2026-07-28. **Verified against the repo at that date** — every count
and path below was measured, not recalled.

Pip's ruling (2026-07-28): the weekly league and player pages are **retired and
kept hidden, not deleted**, and everything generated before the Friday
2026-07-31 patch-cycle regularisation becomes deliberately-labelled **anomalous
pre-history** — visible in the archive as an explicit anomaly section, not
silently buried. In his words, kept so "the ultra archivists can track it down".
...
```
EPOCH_BOUNDARY = 2026-07-31T00:00:00Z
```
...
- `scripts/weekly-league-manager.py` → `EPOCH_BOUNDARY`, `epoch_for()`
- stamped into the data as an `epoch` object (below)
- read by `public/league/archive.html` → `isAnomalous()`

| week | starts | ends | epoch |
|---|---|---|---|
| … 2025_W41 … 2026_W30 | 2025-10-06 … 2026-07-20 | … 2026-07-26 | `pre-regularisation` (anomalous) |
| **2026_W31** | 2026-07-27 | 2026-08-02 | `pre-regularisation` (anomalous) — **straddles the cut**; it was opened under the old regime |
| **2026_W32** | 2026-08-03 | 2026-08-09 | `regularised` — **the first good week**, opened by the Sunday 2026-08-02 14:00 UTC rollover |

The straddle is deliberate. A week that began before the cut cannot be a clean
...
"epoch": {
  ...
  "anomalous": true,
  "boundary_utc": "2026-07-31T00:00:00Z",
  "reason": "Generated before the 2026-07-31 patch-cycle regularisation, while the weekly rollover was off by one week ... Retained as a record of what the pipeline produced, NOT as a comparable competition result.",
...
## The four things that are actually wrong with the pre-history
...
which asserts the week opened at exactly Sunday 14:00:00 UTC and at one minute
either side. That test also runs as the first step of the rollover workflow, so
a regression fails the rollover instead of publishing a wrong week quietly.
...
| `public/leaderboard/data/weekly/current.json` | 1 | Currently `2026_W31`, `epoch.anomalous: true`. Replaced by `2026_W32` at the 2026-08-02 rollover. |
...
| path | role |
|---|---|
| `scripts/weekly-league-manager.py` | The producer. `EPOCH_BOUNDARY`, `epoch_for()`, `league_week_start()`, `rebuild_archive_index()`, `SEED_PROVENANCE`. |
| `scripts/test-weekly-league-boundary.py` | 34 assertions pinning the Sunday-14:00-UTC boundary, the ISO-year straddle, timezone handling, the epoch rule, and that the workflow's cron still matches `ROLLOVER_HOUR_UTC`. |
