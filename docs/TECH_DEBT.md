...
| A8 | ~~**`robots.txt` blocks `/data/`, `/design/`, `/stats/`.**~~ **PARTLY DONE — the visitor-harming two-thirds shipped; the third is a deliberate hold.** The `Disallow` lines for **`/data/` and `/design/` are gone**, and the file now carries a nine-line comment explaining *why* they must not come back (they are runtime subresources; blocking them made Googlebot render the hardcoded fallbacks). That closes the part of this row that made the crawler see a false version number. **The `/stats/` remainder closed 2026-08-06: Pip took the product call and chose RETIRE.** `public/stats/` is deleted (both `index.html` and `competition.html`) and the `Disallow` went with it — a `Disallow` for a path with no files behind it protects nothing. The decision turned out to be easier than the row implied, because the pages were **never reachable at all**: DreamHost's own panel statistics area owns `/stats/` on shared hosting and answers `401 Unauthorized` there, so the noindex question was academic — a crawler that cannot fetch a page cannot read a meta tag on it either. `stats/index.html` was also a near-duplicate of `game-stats/index.html` (**14 differing lines of ~310**: canonical URL, two stylesheet links, whitespace), and the duplication was being paid for — this week's `23%` and frontier-labs corrections each had to be made twice for one visible page. `stats/competition.html` was **not** a duplicate and has no successor; it is recoverable from git history. `public/.htaccess` now attempts a 301 `/stats/` → `/game-stats/`, which **may never fire** — see the caveat below. | **Before, 2026-08-06 production**: `curl https://pdoom1.com/robots.txt \| grep -i '^Disallow'` → `/.netlify/`, `/api/`, `/assets/private/`, `/monitoring/`, `/league/`, `/players/`, `/dev-notes/`, `/stats/`. **No `/data/`, no `/design/`.** **The 401, measured 2026-08-06**: `curl -sI https://pdoom1.com/stats/` → `401`, `WWW-Authenticate: Basic realm="Statistics Area"`; same for `/stats/competition.html`; `/game-stats/` → `200`. | done | **DONE** — 301 needs post-merge verification |
...
| A10 | ~~**`mailto:` links to mailboxes that do not exist.**~~ **FIXED 2026-08-11.** The real address is `team@pdoom1.com`; neither `press@pdoom1.com` nor `contact@pdoom1.com` was ever a real mailbox, so a visitor who wrote to one got silence and reasonably concluded nobody was home. `/press/` was fixed earlier by the press-kit uplift; the two survivors in `public/about/index.html` (`:600` "Send Email", `:611` the `[Mail]` footer link) now point at `team@pdoom1.com`, the address `bug-submit.php:22` hardcodes as its recipient. | **After, 2026-08-11:** `grep -rn "press@pdoom1.com\|contact@pdoom1.com" public/` → **0 hits** (was 2, both in `public/about/index.html`, verified 2026-08-06). `scripts/check-published-emails.py` → PASS. | done | **FIXED 2026-08-11** |
| A11 | **Two published event pages carry mojibake in their `<title>`** — the arrow `→` (U+2192) misdecoded as cp1252 into `U+00E2 U+2020 U+2019`, so the browser tab and every search snippet read *"UK AI Safety Institute â†’ AI Security Institute"*. **Found 2026-08-06 during the scripts audit, and the interesting part is why it is still here.** `scripts/bugfix_pass_20260715.py` fixed exactly these two files on 2026-07-15 and names them at `:192`. `public/data/events.json` is **clean** (0 occurrences), so the daily `sync-events.py` run is not reintroducing it — these two are **orphans no generator rewrites**, the same class as the two `alignmentforum_*` pages CLAUDE.md records as having to be fixed by hand. The fix therefore came off in some later edit and nothing put it back, and nothing will. **This is the reason that script was NOT deleted in the orphan sweep**: it is the only in-repo remedy for a live defect. Fixing the two pages properly is what makes it genuinely dead. | 2026-08-06: 5 occurrences each in `public/events/uk_ai_safety_to_security_2025.html` and `public/events/us_aisi_to_caisi_2025.html`; 0 in `public/data/events.json`; `scripts/bugfix_pass_20260715.py:192` names both filenames. | 0.25h (fix both, then the one-shot can go) | OPEN |

### A8 caveat — the `/stats/` 301 is unverifiable before merge, and will probably lose

`.htaccess` is ignored by Netlify, so **no PR preview can exercise this rule**; and
the docroot is the wrong side of the fight. DreamHost configures its statistics area
above the document root, which is why `public/stats/.htaccess` — an `AuthType None` /
`Satisfy Any` override written by someone earlier for exactly this purpose — deployed
and still lost, as the live 401 proves. A `RewriteRule` in the docroot `.htaccess` is
...
**Verify on production after merge, not on the preview:**

```
curl -sI https://pdoom1.com/stats/
#   301 with Location: /game-stats/   -> the rule took
#   401 realm "Statistics Area"       -> DreamHost still owns the path (expected)
```

Either result is acceptable and neither needs a follow-up fix. Record which one
happened here so the next person does not re-litigate it.
...
| C5 | ~~**`/issues/` form still emails a free-text contact field** to `team@pdoom1.com` via `mailto:`.~~ **FIXED 2026-08-11 — and the row understated the defect.** The `mailto:` was the visible half. The invisible half was worse: the handler `await`ed a `setTimeout(resolve, 1000)` — a **fake network delay** — and then rendered a success-coloured "your report is ready" panel. Nothing was ever transmitted. Anyone who filled the form in, read the confirmation and closed the tab had sent us nothing, and we had no record they had tried. The form now POSTs to `/bug-submit.php` (the same PHP mailer `/bug-report/` uses; recipient hardcoded server-side) and shows a success state **only** on an ok response; a 429 says "wait, your text is still here", and every other failure opens a pre-filled GitHub issue so nothing is lost. The contact field is still free text — it goes to `email` when it parses as an address, and into the mail **body** labelled "not an email address" when it does not, rather than being silently dropped by `FILTER_VALIDATE_EMAIL`; it is never copied into the public GitHub fallback. Two now-false prose claims on the same page ("Nothing here is transmitted automatically", "This form doesn't transmit anything on its own") were rewritten in the same change — `snapshot-copy.py --check` is what surfaced the first one. | **Before:** `public/issues/index.html:693` (`// TODO: Replace with actual Netlify Function or backend endpoint`), `:711` (`setTimeout(resolve, 1000)`), `:716` (the `mailto:`). **After, 2026-08-11:** `grep -n "setTimeout(resolve\|TODO: Replace\|mailto:team@pdoom1.com?subject" public/issues/index.html` → **0 hits**; the page's one write target is `fetch('/bug-submit.php', …)`, declared in `test-escaping.js`'s `GUARDED`. | done | **FIXED 2026-08-11** |
| C6 | **DMARC passes on SPF alone; DKIM fails on every message, including Google's own outbound.** Two authentication mechanisms exist so that one can fail. Right now only one works, so any event that breaks SPF -- DreamHost renumbering, a change to the `netblocks.dreamhost.com` include, or mail leaving by a different path -- takes DMARC down with it, and the only thing preventing rejection at that point is that the policy is still `p=none`. **This is not currently broken. It is a mechanism with no redundancy, recorded while it is cheap.** Fix is one-time: Admin -> Apps -> Google Workspace -> Gmail -> Authenticate email, generate the key, publish the TXT, start authentication. **Do NOT raise `p=none` to `quarantine` until `dkim=pass` appears in an aggregate report** -- tightening the policy while a single mechanism carries it is how a working mail path gets broken. | Google DMARC aggregate reports for 2026-08-18 and 08-19: 5 messages, 3 sources (`209.85.220.41` Google x5, `208.113.156.243` DreamHost x1), **`spf=pass` and `dkim=fail` on every one**, `disposition=none`. Re-check on any later report, or `curl -s 'https://dns.google/resolve?name=google._domainkey.pdoom1.com&type=TXT'`. | 0.5h | **NEEDS-PIP** (Workspace Admin; tracked on #333) |
...
| D6 | **Inbound-contact intake mechanism** (Pip's call, 2026-07-28: *"if we surface them like this we might get spam hit in the future, make a note (not now) to think about info intake mechanisms, for now, I just need correctness"*). Today the site publishes `team@pdoom1.com` as a plain `mailto:` on several pages, which is harvestable and unfilterable, and the one non-`mailto:` alternative — the website form — is **fail-silent to us** (DreamHost PHP `mail()` drops are invisible to sender and receiver alike). So the two failure modes are "drowned in spam" and "never knew it arrived", and we currently have no defence against either. Options worth weighing when it becomes a problem, not before: per-purpose aliases (`press@`, `bugs@`) so a burned address can be retired without changing the good one; a form that writes to a repo or an issue rather than to SMTP, so delivery is verifiable; plus-addressing per surface for source attribution. **Decide only when volume justifies it** — an unused anti-spam system is worse than none. **OPEN, deliberately deferred.** |
...
- **Delivered-but-unread is a fail-silent surface, and the intake ledger cannot see it.**
  On 2026-08-19 a real piece of feedback arrived through `pdoom1.com/issues/` and was
  delivered to `team@pdoom1.com` in **one second** (`spf=pass`, `dmarc=pass`, confirmed
  in the headers and again in Google's aggregate report for that window). Nobody looked
  at it for **two days**. During those two days `content/campaigns/feedback-intake.md`
  showed no entry since 2026-07-24, and that silence was read as "nobody wrote" -- while
  #321 was simultaneously open on the theory that the pipe was broken. Both readings were
  wrong in the same direction: **the ledger records what someone has entered, not what has
  arrived**, so an unread inbox and a dead endpoint produce an identical ledger.

  The estate's standing rule is that a silently lost message is the worst thing the site
  can do. This is the second-order form of it: nothing was lost, and the effect on the
  person who wrote in was the same as if it had been. **A guard for it would have to take
  its input from the mailbox rather than from the ledger** -- a count of messages received
  versus rows logged -- which is the same inside/outside rule the rest of this register
  keeps rediscovering. Not built; recorded so the observation is not re-derived from a
  third incident.
