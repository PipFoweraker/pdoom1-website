...
| A10 | **`mailto:` links to mailboxes that do not exist.** The real address is `team@pdoom1.com`; neither `press@pdoom1.com` nor `contact@pdoom1.com` is a real mailbox, so a visitor who writes to one gets silence and reasonably concludes nobody is home. **`/press/` is fixed** by the press-kit uplift (both of its `press@` links now point at `team@`). **`public/about/index.html:600,611` still carry `contact@pdoom1.com`** and are out of that PR's scope. | `grep -rn "press@pdoom1.com\|contact@pdoom1.com" public/` → 2 hits, both in `public/about/index.html` (`:600` "Send Email", `:611` the `[Mail]` footer link). Verified 2026-08-06. | 0.1h | OPEN |
