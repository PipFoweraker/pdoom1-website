...
|---|---|---|---|---|
| A1 | **No Plausible backups.** Single DreamCompute VPS, no snapshots, no `pg_dump`, no ClickHouse `BACKUP DATABASE`, nothing offsite. The multi-year history Pip wants to animate is one incident from permanent loss. **Interim hedge live** (see A1b): `snapshot-analytics.yml` commits daily aggregates to git. That is a second copy of the *summaries*, not a restorable database, and it only reaches back to 2026-06-23. The real backup is still owed (target 2026-08-05). | `grep -rn "pg_dump\|BACKUP DATABASE" scripts/ ansible/ .github/` → only config lines. `docs/analytics/SELF_HOSTED_PLAUSIBLE.md:509` lists "create monthly backup script" as never done. | 3-5h + a restore drill | **NEEDS-PIP** |
| A1b | **The git hedge cannot reach history older than 2026-06-23.** Daily snapshots use `--period 30d` and the first ran 2026-07-23, so everything before that window exists only on the VPS — exactly the multi-year history Pip wants to animate. `scripts/snapshot-plausible.py --range START:END` exists to backfill it but is **unverified against the live API** (custom-period params were written from the docs, not exercised), and needs a dispatch run with the secret. | `ls public/data/analytics/history/` → first file `2026-07-23.json`; its timeseries starts `2026-06-23`. | ~0.5h (one dispatch + check the file) | OPEN |
...
| B2 | **29 hand-written pages not tokenised.** 147 KB of inline CSS across them, median 5.3 KB, max 16.6 KB (`index.html`). The 2,197 generated pages move with one template edit; this long tail does not. Nav adoption (B1) clawed back a **measured 1,975 bytes** of inline CSS across the 9 converted pages, −1,521 bytes on disk once the script tags and comments are counted. Two commit messages on branch `infra/nav-spine` quoted larger figures (−1.4 KB, −3.2 KB) before the measurement was run; the numbers here are the measured ones. The lesson generalises: deleting duplicated *nav* CSS is a rounding error against 147 KB — the bulk is per-page layout and component styling, and only tokenisation touches that. | design audit; `git show ab00a1cb:<file>` vs working tree | ~15h | OPEN |
