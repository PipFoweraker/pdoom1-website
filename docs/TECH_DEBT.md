...
| D6 | **Inbound-contact intake mechanism** (Pip's call, 2026-07-28: *"if we surface them like this we might get spam hit in the future, make a note (not now) to think about info intake mechanisms, for now, I just need correctness"*). Today the site publishes `team@pdoom1.com` as a plain `mailto:` on several pages, which is harvestable and unfilterable, and the one non-`mailto:` alternative — the website form — is **fail-silent to us** (DreamHost PHP `mail()` drops are invisible to sender and receiver alike). So the two failure modes are "drowned in spam" and "never knew it arrived", and we currently have no defence against either. Options worth weighing when it becomes a problem, not before: per-purpose aliases (`press@`, `bugs@`) so a burned address can be retired without changing the good one; a form that writes to a repo or an issue rather than to SMTP, so delivery is verifiable; plus-addressing per surface for source attribution. **Decide only when volume justifies it** — an unused anti-spam system is worse than none. ~~**OPEN, deliberately deferred.**~~ **NO LONGER DEFERRED — Pip reversed this on 2026-08-15** and made it the top priority: *"If I ever lose a message silently, that's now the worst thing my website can do and it would be better the entire website doesn't exist."* **And this row understated the defect, in the same way C5's did.** It called DreamHost PHP `mail()` drops "invisible to us"; the measured position is worse. `bug-submit.php` writes **no log at all** — the only `file_put_contents` in the file is the throttle timestamp — so **email is the sole record**, and `mail()` returning true means the local MTA accepted the handoff, not that anything arrived. Meanwhile `pdoom1.com` publishes **no SPF, no DKIM and no DMARC** (measured 2026-08-15 against `ns1.dreamhost.com`), so that mail leaves a DreamHost shared IP claiming to be from a domain Google hosts, unauthenticated — the shape of a spoofing attempt. Line 22 asserted `// same domain -> passes SPF on DreamHost`, which is false: SPF authorises sending IPs, not matching domain names, and with no record the result is `none`. **So the two failure modes this row named were the wrong two.** It is not "drowned in spam" versus "never knew it arrived" — it is that a visitor is shown a success panel for a message that may never have been delivered, and nothing anywhere would know. Work in progress on branch `feedback/intake-contract`: contract at `docs/decisions/FEEDBACK_INTAKE_CONTRACT.md`, live state at `docs/decisions/FEEDBACK_INTAKE_SESSION_STATE.md`. **The comment on line 22 is corrected as of 2026-08-16; the missing `-f` envelope sender and the absent DNS records are both still open**, and `scripts/check-mail-auth.py` pins DMARC at `p=none` until the first is fixed. | **IN PROGRESS 2026-08-16** |
