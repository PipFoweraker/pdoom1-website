...
| A9 | **Weekly-league rollover is off by one week.** Cron fires Sunday 14:00 UTC; `get_current_week_info()` derives the week from `now`, so it creates the week that *ends* 10 hours later. `validate_data.py` confirms: "week 2026_W29 is marked is_current but ended 2.4 days ago". 10 weeks of green checkmarks, all wrong. This is the #126 false positive, proven. | `scripts/weekly-league-manager.py:73`, `weekly-league-reset.yml` | 0.5h | **FIXED 2026-07-28.** Run-time → week mapping is now explicit (`league_week_start()`), pinned by `scripts/test-weekly-league-boundary.py` (asserts the week opened at exactly Sun 14:00:00 UTC and one minute either side), which also runs as the **first** step of the rollover workflow so a regression fails the rollover instead of publishing a wrong week. The ten wrong weeks are kept and labelled — see `docs/LEAGUE_EPOCH_ANOMALY.md`. |
