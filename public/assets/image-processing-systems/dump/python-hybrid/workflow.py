#!/usr/bin/env python3
"""
Hybrid Batch Image Processing Workflow for pdoom1-website Assets

Enhanced workflow that processes images with:
- Multiple aspect ratios for web (square, wide, standard, portrait, original)
- Optimized web compression (fast loading, high visual quality)
...
from process_image import HybridImageProcessor


class HybridAssetWorkflow:
    """Enhanced workflow for processing image assets with multiple aspect ratios."""
    ...
    def scan_new_images(self) -> List[Path]:
        ...
        new_images = [img for img in all_images if img.stem not in processed]
        
        return sorted(new_images)
    
    def process_cat_images(self, image_path: Path, force: bool = False) -> Dict:
        """
        Process cat images with multiple aspect ratios and optimized compression.
        
        Web outputs:
        - Multiple aspect ratios: square, wide, standard, portrait, original
        - Optimized formats: WebP (primary), JPEG (fallback)
        - Multiple sizes: thumbnail, small, medium, large
        - Fast loading with high visual quality
        ...
        game_outputs = processor.process_image(
            ...
            formats=['png', 'webp'],
            sizes=['game-small', 'game-medium', 'game-large', 'game-xlarge'],
            aspect_ratios=['original'],  # Preserve original for game
            target='game'
        )
        
        # Combine outputs
        all_outputs = {**web_outputs, **game_outputs}
        ...
        print(f"[OK] Generated {len(all_outputs)} outputs ({total_size_kb:.1f}KB total)")
        
        return result
    
    def run(self, force: bool = False, scan_only: bool = False):
        """Run the complete workflow."""
        print("[INFO] Starting hybrid asset processing workflow...")
        print("[INFO] Web: Multiple aspect ratios with optimized compression")
        print("[INFO] Game: High-quality assets with original aspect ratio")
...
def main():
    parser = argparse.ArgumentParser(
        description='Hybrid batch image processing workflow',
...
Examples:
  ...
  python workflow.py --force
    ...
    workflow = HybridAssetWorkflow(dump_dir, output_dir)
