...
- **The homepage no longer states it in prose at all (2026-08-24).** v0.14.3 shipped
  ...
  `public/index.html` now writes all of them from `version.json`, and the shipped
  HTML carries placeholders that name no operating system. **Guarding typed prose
...
- **Known hole, left open deliberately: `SOFT_QUALIFIER` matches any em dash**, so
  ...
  the homepage unflagged through a release that shipped no macOS build. That sentence
  is gone, but the hole is not closed -- narrowing it needs every genuine
  "macOS -- coming soon" on the site re-checked first.
